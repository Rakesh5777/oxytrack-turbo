...
interface BreadcrumbRouterDomLinkProps extends React.ComponentProps<typeof Link> {
  asChild?: boolean;
}

const BreadcrumbRouterDomLink = React.forwardRef<HTMLAnchorElement, BreadcrumbRouterDomLinkProps>(({ asChild, className, to, ...props }, ref) => {
  const Comp = asChild ? "span" : Link;

  return <Comp ref={ref} to={to} className={cn("transition-colors hover:text-foreground", className)} {...props} />;
});

BreadcrumbRouterDomLink.displayName = "BreadcrumbRouterDomLink";
